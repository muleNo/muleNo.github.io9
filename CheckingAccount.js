 ...
 *
 * Provides the functionalities that Checking Account provides such withdraw.
 */
class CheckingAccount extends Account {
   ...
   */
  withdraw(amount) {
    if (amount <= 0) throw new RangeError(`Amount should be greater than zero`);
    if (Math.abs(this._balance - amount) > this._overDraftLimit)
      // if the person tries to withdraw beyond the overdraft limit allowed
      throw new RangeError(
        `You are not allowed to withdraw beyond the allowed overdraft limit`
      );
